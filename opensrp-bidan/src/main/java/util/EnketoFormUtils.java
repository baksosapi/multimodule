...
import static org.smartregister.bidan.sync.BidanClientProcessor.CONTACT_SCREENING;
import static org.smartregister.bidan.sync.BidanClientProcessor.DIAGNOSIS_EVENT;
import static org.smartregister.bidan.sync.BidanClientProcessor.TREATMENT_INITIATION;
import static org.smartregister.util.Log.logInfo;
import static org.smartregister.bidan.constant.BidanConstants.KEY.ATTRIBUTE_DATEREMOVED;
...
import static org.smartregister.bidan.constant.BidanConstants.BIDAN_ID;
            ...
            try {
                ...
                if (event.getEventType() != null) {
                    if (event.getEventType().equals(DIAGNOSIS_EVENT)) {
                        ...
                        Client client = gson.fromJson(json.toString(), Client.class);
                        Client c = formEntityConverter.getClientFromFormSubmission(formSubmission);
                        client.addIdentifier(BIDAN_ID, c.getIdentifier(BIDAN_ID));
                        client.setAddresses(c.getAddresses());
                        saveClient(client);
                    } else if (event.getEventType().equals(BidanConstants.EVENT.REMOVE_PATIENT) || event.getEventType().equals(BidanConstants.EVENT.TREATMENT_OUTCOME)) {
                        JSONObject json = eventClientRepository.getClientByBaseEntityId(event.getBaseEntityId());
                        Client client = gson.fromJson(json.toString(), Client.class);
