...
import org.smartregister.bidan.database.model.BMIRecord;
import org.smartregister.bidan.database.model.BMIRecordWrapper;
...
public class RenderBMIHeightChartCardHelper extends BaseRenderHelper {
            ...
            public void run() {
                try {
                    ...
                    if (bmiRecordWrapper.getBmiRecords().size() > 0) {
                        ...
                        if (isWeightOnly) {
                            TextView title = view.findViewById(R.id.bmi);
    ...
    private void processBMILineChartView(List<BMIRecord> bmiList, boolean isWeightOnly, Integer maxValue, List<PointValue> values, View view, Map<String, String> patientDetails) {
        LineChartView bmiLineChartView = view.findViewById(R.id.bmiHeightLineChartView);
        if (bmiLineChartView != null) {
            ...
            TextView bmiStartTextView = view.findViewById(R.id.bmiStartTextView);
            ...
            TextView treatmentStartDateTextView = view.findViewById(R.id.treatmentStartDateTextView);
            treatmentStartDateTextView.setText(Utils.formatDate(org.smartregister.util.Utils.toDate(patientDetails.get(BidanConstants.KEY.TREATMENT_INITIATION_DATE), true), "MMM yyyy") + " (" + context.getString(R.string.treatment_start) + ")");
            TextView bmiLastTextView = view.findViewById(R.id.bmiLastTextView);

            TextView treatmentEndDateTextView = view.findViewById(R.id.treatmentEndDateTextView);
            if (values.size() > 1) {
