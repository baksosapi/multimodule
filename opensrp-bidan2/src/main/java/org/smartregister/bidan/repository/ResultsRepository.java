        ...
        try {
            ...
            if (cursor != null && cursor.moveToFirst()) {
                do {
                    ...
                    calendar.setTimeInMillis(cursor.getLong(cursor.getColumnIndex(BidanConstants.KEY.DATE)));
                    result.setDate(calendar.getTime());
                    result.setResult1(key);
                    result.setValue1(cursor.getString(cursor.getColumnIndex(VALUE1)));

                    if (key2 != null && !key2.isEmpty() && key2.equals(BidanConstants.RESULT.ERROR_CODE)) {
